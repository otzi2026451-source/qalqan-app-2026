    ...
    <section class="page grid grid-2">
      <div class="card">
        ...
        <form class="form" id="credit-form">
          ${field('creditAmount', 'Сумма', 'number', '500000', '500000')}
          ${selectField('creditTerm', 'Срок', [{ value: 6, label: '6 месяцев' }, { value: 12, label: '12 месяцев' }, { value: 24, label: '24 месяца' }])}
      ...
      <div class="card">
        ...
        ${credit ? `
          ...
          <div class="stat"><span>Ежемесячный платеж</span><strong>${money(credit.monthly)}</strong></div>
          <div class="btn-row" style="margin-top:18px;">
            <button class="btn btn-outline" data-pay-credit>Оплатить месяц</button>
            <button class="btn btn-danger" data-close-credit>Погасить полностью</button>
          </div>
        ` : '<div class="empty">Кредита пока нет</div>'}
      </div>
    </section>
  `;
}
    ...
    <section class="page grid grid-2">
      <div class="card">
        ...
        <form class="form" id="deposit-form">
          ${field('depositAmount', 'Сумма', 'number', '200000', '200000')}
          ${selectField('depositTerm', 'Срок', [{ value: 3, label: '3 месяца' }, { value: 6, label: '6 месяцев' }, { value: 12, label: '12 месяцев' }])}
      ...
      <div class="card">
        <h2>Мой депозит</h2>
        ${deposit ? `
          <div class="stat"><span>Сумма</span><strong>${money(deposit.amount)}</strong></div>
          <div class="stat"><span>Ставка</span><strong>${deposit.rate}%</strong></div>
          <div class="stat"><span>Доход</span><strong>${money(deposit.profit)}</strong></div>
          <button class="btn btn-outline" style="margin-top:18px;" data-topup-deposit>Пополнить на 50 000 ₸</button>
        ` : '<div class="empty">Депозита пока нет</div>'}
      </div>
    </section>
  `;
}
...
function renderQR() {
  const user = requireUser();
  if (!user) return '';
  const cells = Array.from({ length: 81 }, (_, i) => i % 4 === 0 || i % 7 === 0 ? 'qr-cell' : 'qr-cell empty');
  return `
    <section class="page grid grid-2">
      <div class="card">
        <h2>QR-оплата</h2>
    ...
    <section class="page">
      <div class="card">
        ...
        <div class="grid grid-3" style="margin-bottom:16px;">
          ...
          ${selectField('historyType', 'Тип', [{ value: '', label: 'Все' }, { value: 'TRANSFER', label: 'Переводы' }, { value: 'PAYMENT', label: 'Платежи' }, { value: 'CARD', label: 'Карты' }, { value: 'CREDIT', label: 'Кредиты' }, { value: 'DEPOSIT', label: 'Депозиты' }])}
        ...
        <div class="btn-row" style="margin-bottom:16px;"><button class="btn btn-outline" id="export-history">Скачать CSV</button></div>
        <section id="history-list">${renderTxList(userTransactions())}</section>
      </div>
    </section>
  `;
}
